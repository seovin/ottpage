...
if(($bn == 'notice' || $bn == 'programs') && $_SESSION['permit']!=3) // 전에 만들었던 권한 값이 3이 아니라면 접근 불가
{
  ...
  $db->query = "insert into ".$bn." values(null, '".$_SESSION['id']."', '".$pass[1]."', '".date('Y-m-d H:i:s')."', '".$title."', '".$content."',0)";
  ...
  {
  echo "<script>alert('글이 정상적으로 업로드 되었습니다.');location.replace('/page/".$bn."')</script>";
  }
...
else if($bn=='board')
{
  ...
  $db->query = "insert into ".$bn." values(null, '".$_SESSION['id']."', '".$pass[1]."', '".date('Y-m-d H:i:s')."', '".$category."', '".$title."', '".$content."',0)";
...
$db->query = "insert into ".$bn." values(null, '".$_SESSION['id']."', '".$pass[1]."', '".date('Y-m-d')."', '".date('H:i:s')."', '".$category."', '".$title."', '".$content."', '".$link."', '".$ott."', '".$upfile."', ".$genre.")";
