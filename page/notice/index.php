...
$db->query = "select count(*) from notice";
$db->DBQ();
...
if(isset($_GET['searchColumn']) && isset($_GET['searchText']))
{
  ...
  $db->query ="select no, datetime, title, id, hit from notice where ".$searchColumn." like '%".$searchText."%' order by no desc limit ".$limit.", ".$onepage;
}
else {
  $db->query = "select no, datetime, title, id, hit from notice order by no desc limit ".$limit.", ".$onepage;
}
    ...
    } else
    {
		...
		<div class='wrap'>
			  <div class='no'><a href='./view.php?v=".$data[0]."'>".$data[0]."</a></div>
				<div class='title'><a href='./view.php?v=".$data[0]."'>".$data[2]."</a></div>
				<div class='name'>".$data[3]."</div>
        <div class='datetime'>".$data[1]."</div>
        <div class='hit'>".$data[4]."</div>
